...
function Review({ title, body, author, rating, className, ...props }) {
  ...
  return (
    <figure
      className={clsx(
        'animate-fade-in rounded-3xl bg-white opacity-0 shadow-md shadow-gray-900/5 flex flex-col overflow-hidden',
        className
      )}
      style={{ animationDelay }}
      {...props}
    >
      <blockquote className="p-8 text-xl leading-8 tracking-tight text-gray-900">
        <p>{`“${body}”`}</p>
      </blockquote>
...
function ReviewGrid() {
  ...
  // changed it to always true so that it always shows the animation
  let isInView = true;
  ...
  return (
    <div
      ref={containerRef}
      // don't know why -mx-4 is needed but it was messing up mobile view
      // className="relative -mx-4 mt-16 grid h-[49rem] max-h-[150vh] grid-cols-1 items-start gap-8 overflow-hidden px-4 sm:mt-20 md:grid-cols-2 lg:grid-cols-3"

      // I removed lg:grid-cols-3 because I don't have that many reviews plus I want them to be wider in general because of new design

      className="relative grid h-[49rem] max-h-[150vh] grid-cols-1 items-start gap-8 overflow-hidden  md:grid-cols-2"
    >
      {isInView && (
        <>
          <ReviewColumn
